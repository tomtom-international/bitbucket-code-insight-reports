...
from bitbucket_code_insight_reports.bitbucket_code_insight_reports import Report
...
def test_init(auth, base_url, project_key, repo_slug, commit_id, key, title, description, path, line, message, gen_annotations):
    result = "PASS"
    test_annotation = gen_annotations(path, line, message)
    test_annotation_string = json.dumps(test_annotation)

    test_report_url = base_url + "/rest/insights/1.0/projects/" + project_key + "/repos/" + repo_slug + "/commits/" + commit_id + "/reports/" + key

    test_report = Report(auth, base_url, project_key, repo_slug, commit_id, key, title, description, result, test_annotation_string)
